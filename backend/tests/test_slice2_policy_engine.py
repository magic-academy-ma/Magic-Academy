...
def test_every_state_signal_reaches_policy_effect(signal_type, metric):
    ...
    # WAIT의 canonical 기본 효과(hunger/fatigue)와 metric이 겹칠 수 있으므로
    # 이 signal이 실제로 만든 효과(STATE_ 접두 rule_id)만 식별해 검증한다.
    effects = [
        effect
        for effect in result.effect_candidates
        if effect.metric == metric and effect.rule_id.startswith("STATE_")
    ]
    assert len(effects) == 1
...
def _make_rel_signal(signal_type, intensity, target_id):
    return RelationshipSignal(
        signal_type=signal_type, intensity=intensity, target_agent_id=target_id
    )
...
def _make_eval_input(runtime_results, rel_snapshots=None, agent_snapshots=None):
    ...
    return PolicyEvaluationInput(
        ...
        if agent_snapshots is not None
            ...
            str(AGENT_A): AgentSnapshot(
                agent_id=str(AGENT_A), hunger=50, fatigue=20, stress=10, satisfaction=50
            ),
            str(AGENT_B): AgentSnapshot(
                agent_id=str(AGENT_B), hunger=50, fatigue=20, stress=10, satisfaction=50
            ),
        },
        relationship_snapshots=rel_snapshots
        if rel_snapshots is not None
        else [
            RelationshipSnapshot(
            ...
            RelationshipSnapshot(
                source_agent_id=str(AGENT_B),
                target_agent_id=str(AGENT_A),
                trust=15,
                tension=3,
            ),
        ],
...
def test_ab_signal_does_not_affect_ba():
    ...
    ba_trust = [
        e
        for e in result.effect_candidates
        if e.metric == "trust" and e.source_agent_id == str(AGENT_B)
    ]
...
def test_mood_up_produces_state_effect():
    ...
    results = [
        _make_runtime_result(
            ...
            state_signals=[
                StateSignal(
                    signal_type=StateSignalType.MOOD_UP,
...
def test_invalid_relationship_signal_keeps_valid_effects():
    ...
    results = [
        _make_runtime_result(
            ...
            rel_signals=[
                ...
                _make_rel_signal(
                    RelationshipSignalType.TRUST_UP, SignalIntensity.MEDIUM, AGENT_X
                ),
            ],
            state_signals=[
                StateSignal(
    ...
    assert {
        (effect.metric, effect.target_agent_id) for effect in result.effect_candidates
    } == {
        ("trust", str(AGENT_B)),
        ("mood", None),
        ("fatigue", None),
...
def test_closeness_preview_allows_negative_value():
    ...
    closeness_effect = next(
        effect for effect in result.effect_candidates if effect.metric == "closeness"
    )
...
def test_conflicting_relationship_signals_are_both_rejected():
    ...
    results = [
        _make_runtime_result(
            ...
            rel_signals=[
                ...
                _make_rel_signal(
                    RelationshipSignalType.TRUST_DOWN, SignalIntensity.MEDIUM, AGENT_B
                ),
            ],
        )
    ]
    ...
    assert result.status == PolicyStatus.PARTIAL
    assert [
        effect for effect in result.effect_candidates if effect.metric == "trust"
    ] == []
...
def test_multiple_deltas_on_same_pair_are_summed():
    ...
    ab_trust = [
        c for c in committed if c.source_agent_id == "a" and c.metric == "trust"
    ]
...
def test_merged_candidates_join_rule_id_and_reason():
    ...
    assert committed[0].effect_ids == ("e1", "e2")
...
def test_duplicate_state_signals_are_deduplicated():
    ...
    signal = StateSignal(
        signal_type=StateSignalType.STRESS_DOWN, intensity=SignalIntensity.LOW
    )
