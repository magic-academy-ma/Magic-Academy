...
[Backend API]  FastAPI — 도메인 중심 모듈 구조
     ...
     ↓
[Tick Engine]  TickScheduler(APScheduler in-process) + SimulationTickService
               + TickOrchestrator
     ↓
[Event Master Agent]  Sonnet 4.6 — 매 tick 일반 이벤트 생성
     ↓
[Magic Layer]  Haiku 4.5 — 이벤트 변환 + 조건 기반 특수 사건 서술
     ↓
[Agent Runtime]  Slice 1 순차 batch, 확장 단계 제한 병렬 실행
...
| Agent 실행 | Slice 1 순차 batch, 확장 단계 asyncio.gather + Semaphore 제한 병렬 |
...
| 실시간 통신 | FastAPI 내장 WebSocket — commit 이후 변경분 push. 구체 경로와 Schema는 비공개 API 명세에서 관리 |

### 시뮬레이션 실행 환경

| 항목 | 채택 |
| --- | --- |
| Tick 실행 | TickScheduler(APScheduler in-process) + SimulationTickService + TickOrchestrator |
