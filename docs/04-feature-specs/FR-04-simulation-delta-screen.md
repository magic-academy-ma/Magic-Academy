...
- [x] 상태와 관계 delta를 각각 렌더링한다.
- [x] 양수·음수·0과 clamp 결과를 표시한다.
- [x] UUID 대신 Agent 이름을 표시한다.
- [x] A→B와 B→A를 다른 방향으로 표시한다.
- [x] 성공한 Tick의 실제 `after`를 표시한다.
- [x] 로딩·빈 결과·401·403·409·5xx를 처리한다.
